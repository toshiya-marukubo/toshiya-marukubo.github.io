...
export class Sketch {
  ...
  getGeometry(size) {
    ...
    const base = new THREE.BoxGeometry(size * block * 0.5, size * 0.6, size, 1, 1, 1);
    
    lego.push(base);

    for (let x = 0; x < block; x++) {
      for (let z = 0; z < 2; z++) {
        const nx = size * 0.5 * (x - block * 0.5) + size * 0.25;
        const nz = size * 0.5 * (z - 2 * 0.5) + size * 0.25;
        const geo = new THREE.CylinderGeometry(size * 0.156, size * 0.156, size * 0.106, 36);
    ...
    geometry.applyMatrix4(new THREE.Matrix4().makeTranslation(0, 0, size * 1.5));
...
class Shape {
  ...
  initialize() {
    ...
    this.mesh.rotation.y = Math.floor(this.index % 4) * Math.PI / 2;
  ...
  render() {
    ...
    switch (intT) {
      ...
      case 3:
        ...
        moveY = Utilities.map(st, 0, 1, 0, -this.size * 0.6 * 4);
    ...
    */
    ...
    const newV = this.position.clone().add(new THREE.Vector3(0, moveY, 0)).multiplyScalar(scale);

    this.mesh.rotation.set(0, Math.floor(this.index % 4) * Math.PI / 2 + rotate, 0);
