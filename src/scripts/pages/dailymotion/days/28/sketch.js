...
export class Sketch {
  ...
  getGeometry(size) {
    const geometry = new THREE.BoxGeometry(size * 0.2, size, size * 0.8, 1, 1, 1);
  ...
  updateEquipments(t) {
    ...
    this.camera.position.set(
      Math.cos(t) * this.dist * 0.9,
      Math.abs(Math.sin(t)) * 100 + 100,
      Math.sin(t) * this.dist * 0.9
    );
...
class Shape {
  ...
  initialize() {
    ...
    if (this.index % 2 === 0) this.mesh.rotation.y = Math.PI / 2;
  ...
  render() {
    ...
    let st, moveX, moveZ, rotate = Math.PI;
    switch (intT) {
      case 0:
        st = this.sketch.ease(t % 1);

        if (this.index % 2 === 0) {
          moveX = Utilities.map(st, 0, 1, 0, this.size);
          moveZ = 0;
        } else {
          moveX = 0;
          moveZ = Utilities.map(st, 0, 1, 0, this.size);
        }
      ...
      case 1:
        ...
        if (this.index % 2 === 0) {
          moveX = Utilities.map(st, 0, 1, this.size, this.size * 2);
          moveZ = 0;
          rotate = Utilities.map(st, 0, 1, 0, Math.PI / 2);
        } else {
          moveX = 0;
          moveZ = Utilities.map(st, 0, 1, this.size, this.size * 2);
          rotate = Utilities.map(st, 0, 1, 0, -Math.PI / 2);
        }
    ...
    const newV = this.position.clone()
                   .add(new THREE.Vector3(moveX, 0, moveZ));

    if (this.index % 2 === 0) {
      this.mesh.rotation.y = Math.PI / 2 + rotate;
    } else {
...
class CreateTexture {
  ...
  drawTexture() {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      const x = Math.cos(rad) * this.canvas.width / 2;
      const y = Math.sin(rad) * this.canvas.height / 2;
