...
export class Sketch {
  ...
  setupShapes() {
    ...
    this.size = Math.floor(this.scale) * 0.5;
  ...
  getGeometry(size) {
    const geometry = new THREE.CylinderGeometry(size, size, size, 3, 1);
      ...
      .applyMatrix4(
        new THREE.Matrix4()
          .makeTranslation(
            0,
            size / 2,
            -(Math.cos(30 * Math.PI / 180) * size / 2) * 2)
          );
...
class Shape {
  ...
  constructor(params) {
    ...
    this.moveDist = (Math.cos(30 * Math.PI / 180) * this.size) * 2;
    
    this.position = new THREE.Vector3(params.position.x, params.position.y, params.position.z + this.moveDist / 2);
  ...
  render() {
    ...
    let st;
    let moveZ = 0, rotateZ = 0;

    switch (intT) {
      case 0:
        st = this.sketch.ease(t % 1);

        rotateZ = Utilities.map(st, 0, 1, 0, Math.PI / 3 * 2);
        moveZ = Utilities.map(st, 0, 1, 0, this.moveDist);
    ...
    const newV =
      this.position.clone()
        .add(new THREE.Vector3(0, 0, -moveZ));

    this.mesh.position.set(newV.x, newV.y, newV.z);
    this.mesh.rotation.x = rotateZ;
  }
}
