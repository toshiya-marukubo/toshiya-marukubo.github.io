...
export class Sketch {
  ...
  updateEquipments(st) {
    ...
    */
    ...
    let dist;
    if (st < 0.33) {
      dist = Utilities.map(st, 0, 0.33, this.scale * 2, this.scale);
    } else if (st >= 0.33 && st < 0.66) {
      dist = Utilities.map(st, 0.33, 0.66, this.scale, this.scale);
    } else {
      dist = Utilities.map(st, 0.66, 1, this.scale, 0);
    }
...
class Shape {
  ...
  initialize() {
    ...
    geometry.applyMatrix4(new THREE.Matrix4().makeTranslation(-this.size / 2, 0, 0));
  ...
  render(t, st) {
    ...
    if (st < 0.33) {
      rotate = Utilities.map(st, 0, 0.33, 0, 0);
    } else if (st >= 0.33 && st < 0.66) {
      rotate = Utilities.map(st, 0.33, 0.66, 0, Math.PI / 2);
    } else {
      rotate = Utilities.map(st, 0.66, 1, Math.PI / 2, Math.PI / 2);
    }

    if (this.index === Math.floor(this.sketch.shapes.length / 2)) {
      this.mesh.rotation.y = Math.PI + rotate;
    } else {
