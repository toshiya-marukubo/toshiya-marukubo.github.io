...
export class Sketch {
  ...
  initialize() {
    ...
    this.ease2 = Ease.returnEaseFunc('linear');
    this.ease3 = Ease.returnEaseFunc('easeOutBounce');
    this.ease4 = Ease.returnEaseFunc('easeInOutElastic');
    this.ease5 = Ease.returnEaseFunc('easeOutExpo');
  ...
  setupCamera() {
    ...
    this.camera.position.set(0, this.dist * 0.15, this.dist * 0.2);
  ...
  setupLights() {
    ...
    this.spotLight.position.set(0, 100, -this.dist * 2);
  ...
  setupRest() {
    ...
    this.scene.fog = new THREE.Fog(0xFFFFFF, 0, this.dist);
  ...
  setupShapes() {
    ...
    if (num === 1) {
      this.shapes.push(new Shape(this, 0, 0, 0, this.size, this.maxDist, 0, false));
...
class Shape {
  ...
  initialize() {
    // geometry
    const geometry = new THREE.SphereGeometry(this.size, 36);
  ...
  render() {
    ...
    //const floatT = Math.floor(t % this.timeNum);
    const st = this.sketch.ease5(t % 1);

    const x = Utilities.map(st, 0, 1, this.position.x, this.others.position.x);
    const y = Utilities.map(st, 0, 1, this.position.y, this.others.position.y);
    const z = Utilities.map(st, 0, 1, this.position.z, this.others.position.z);
