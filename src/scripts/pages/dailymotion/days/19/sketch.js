...
class Shape {
  ...
  initialize() {
    // geometry
    const geometry = new THREE.BoxGeometry(this.size, this.size * 21, this.size * 20, 1, 1, 1);
  ...
  render() {
    ...
    switch (intT) {
      ...
      case 1:
        ...
        scale = Utilities.map(st, 0, 1, 1, 5);
      ...
      case 2:
        ...
        rotate = 0;
        scale = Utilities.map(st, 0, 1, 5, 1);
      ...
      case 5:
        ...
        rotate = 0;
        scale = 1;
        moveY = Utilities.map(st, 0, 1, 1000, 0);
    ...
    const addV = new THREE.Vector3(0, moveY, 0);
    const newV = this.position.clone().multiplyScalar(scale).add(addV);

    this.mesh.rotation.set(rotate, 0, 0);
