...
export class Sketch {
  ...
  setupCanvas() {
    ...
    this.canvas.ariaLabel = 'The lines are moving on the heart.';
  ...
  setupSizes() {
    ...
    this.gui.params.scale = Math.min(window.innerWidth / 1.5, window.innerHeight / 1.5);
  ...
  setupShapes() {
    ...
    this.points = {
      a: Points.heart(this.vector, 1080),
    };
...
class Shape {
  ...
  getTime(i) {
    ...
    const scaledTime = t * this.timeScale - this.index / 2 / Math.PI * 2;
