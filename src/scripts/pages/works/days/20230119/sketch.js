...
class Shape {
  ...
  initialize() {
    ...
    geometry.applyMatrix4(new THREE.Matrix4().makeTranslation(0, this.size / 2, -this.size / 2));
  ...
  render() {
    ...
    let st, stt, moveY = 0, rotateX = 0;

    switch (IntT) {
      case 0:
        st = this.sketch.easeOutBounce(t % 1);
        stt = this.sketch.ease(t % 1);
          
        moveY = Utilities.map(stt, 0, 1, 0, -this.size);
        rotateX = Utilities.map(st, 0, 1, 0, Math.PI / 2);
