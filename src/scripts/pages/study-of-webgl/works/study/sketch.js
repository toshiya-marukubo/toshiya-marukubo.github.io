...
export class Sketch {
  ...
  setupSizes() {
    ...
    this.scale = Math.floor(Math.max(this.gui.params.scale * ratio, 50));
  }
  ...
  setupShape() {
    ...
    mesh.position.set(0, -this.scale / 2, 0);
    ...
    //this.shapes = this.getSquareGrid(2, 50);
    this.shapes = this.getHexGrid(this.gui.params.number, size); 
  }
  ...
  getHexGrid(num, size) {
    ...
    for (let i = 0; i < vectors.length; i++) {
      ...
      const s = new Shape(this, x, 0, y, size, d, i, false);
...
class Shape {
  ...
  initialize() {
    // geometry
    const geo1 = this.getGeometry('cylinder', this.size, 6);
  ...
  render(time, scaledTime) {
    ...
    this.mesh.scale.set(1, scale * 5, 1);
    this.mesh.rotation.set(0, angle2, 0);
  }
}
