...
const getFiles = () => {
  const objs = {};
  const files = globule.find(['./src/scripts/*.js', './src/styles/days.styl']);
...
for (let i = 0; i < htmls.length; i++) {
  ...
  // continue if included index-components directory
  if (html.match(/index-components/)) {
    continue;
  }

  if (html.match(/summary-components/)) {
    continue;
  }
